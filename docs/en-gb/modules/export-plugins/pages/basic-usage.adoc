...
This tutorial will introduce you to the catalogue export. We will develop a small plugin that exports variations and you will learn how to create and work with a catalogue to allow your users to map their data to a predefined format. If you are just getting started developing plugins for plentymarkets, we recommend to work through the xref:getting-started.adoc[getting started section] first.
...
In the following section, we will create a template and extend its functionality step by step until we cover all fundamental features.

=== Basics

To get started, we will create a very simple template with just a single result field and no additional logic like filtering or data manipulation. To do so, we need to create a template provider class. This class needs to extend the abstract base class `AbstractGroupedTemplateProvider` *ADD LINK?*. It will be used to fill a template with data when it gets booted. A template provider can be used for the booting process of multiple templates, so you don’t need to create multiple classes for similar templates (e.g. you create templates for different product categories of the same marketplace).

For simplicity reasons, we will first provide the example code and the look into what each method does after that. The example code can be copied from here:

*COMMENT: SHOULD BE WRITTEN IN A CODE BLOCK???*
...
We can now map a source to the field, e.g. the *Variation ID*, so we have a fully exportable catalogue. We can test this by pressing the preview button.

.Preview of the export
image::export-preview.png[Preview of the export,640,360]

Now that we have a working example template that implements the most basic functionality, we can go more deeply into the specific topics that are definable through a template.
...
* `getTemplateGroupContainer()` *LINK*?
* `getFilterContainer()` *LINK*?
* `getCustomFilterContainer()` *LINK*?
* `isPreviewable()` *LINK*?
...
As we can see here, we first define our container, then we define a group and at last a field. In general, this process will always be the same. That means we will always define groups, and we will always add fields to them. What will change depending on the use case is the type of the field. As we can see in the example, we define an instance of the class xref:stable7@docs-raw::Catalog.adoc#catalog_models_simpletemplatefield[SimpleTemplateField]. Depending on the specific use case, we would have to use a different one.

Currently available are:
* *xref:stable7@docs-raw::Catalog.adoc#catalog_models_simpletemplatefield[SimpleTemplateField]* - Defines a result field that can be filled through a source field out of the plentymarkets system or a custom value
* *xref:stable7@docs-raw::Catalog.adoc#catalog_models_complextemplatefield[ComplexTemplateField]* - Defines a result field that will be filled by a value out of a predefined list depending on a condition (e.g. a specific category name depending on a linked category in plentymarkets)
* *xref:stable7@docs-raw::Catalog.adoc#catalog_models_combinedtemplatefield[CombinedTemplateField]* - Defines two result fields. A complex one and a simple one. The complex field will be filled depending on which source fields were used to fill the simple field (e.g. filling a barcode type depends on which source field was used to fill the barcode field).
...
The complex field should be used whenever you have a list of values. The typical example is linking a variation to a category. A marketplace will not let you define new categories on the fly, instead you have to choose an existing one and link your variation to it. Therefore, we have a finite amount of valid values which we can provide in a complex field.

The combined field is true to its name. You should use it, if you have a complex field which is logically bound to a simple field. Maybe a marketplace expects a barcode and the specific type of that barcode. Therefore, those two fields should be combined into one field in order to determine the barcode and also its type.
...
Now we can have a look at all the parameters in a simple template field. The following parameters are available:

*exportKey*: The key that will be present in the export result.
*key*: unique identifier, which will be used to work with the field. Therefore, the exportKey can be changed if necessary without users having to redo all the mappings.
*label*: The string that will be displayed in the UI.
*isRequired*: Will mark fields with an asterisk (*) in the UI and display a warning message, if the catalogue is saved when a required field is not filled.
...
*isArray*: Defines, which behaviour is applied in the export for this field. If put to true, all mapped sources will be used to fill an array for this field at the end. If it is set to false, only the first non-null value will be written into the field and all following sources will be skipped.
...
Now we can add the fields *variationName*, *price* and *sku*, since these are fields that expect an individual value and therefore should be defined as simple fields. The definition for those will look identical to our first example. The only difference is that these should be required.
...
To make the usage of this catalogue more convenient to the user, we can add the net stock of the variation as default source for the stock field. The source can either be loaded through the xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_fieldgrouprepositorycontract[FieldGroupRepository] or you can just go to the catalogue UI, open the dev tools and map the source yourself.
...
The first method getValueById() should be self-explanatory. It will receive an identifier and should return the value that matches the identifier. The value has to be wrapped in an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_models_catalogmappingvalue[CatalogMappingValue].

The next method getValuesByParentId is called to load a single level of values. If the UI of a catalogue is opened, an initial request is sent that requests all values with parentId = null. If our provider has nested values, it can happen that a user clicks on a parent value in the UI, which triggers a call to this method that includes the clicked value’s ID. In this case, we should return the nested values for that value in an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_containers_catalogmappingvaluecontainer[CatalogMappingValueContainer].
...
Since we’ve already covered the other two types, this one should be easy. It is actually a combination of the previous two types. Just as in the last step, we need an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogmappingvalueprovidercontract[CatalogMappingValueProviderContract]. This time, it will provide different types of barcodes. To keep it simple, let’s only use GTIN and ISBN for now. The provider class could look like this:
...
Additionally, we need an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_containers_catalogtemplatefieldcontainer[CatalogTemplateFieldContainer] which will contain all simple fields of our combined field. The logic is the following:
...
*_Note:_* The behaviour of isArray in the context of CombinedFields is not strictly defined yet. We suggest using false for now until this will have changed in the future.
...
The first type are the static ones. In the scope of the project we will simply refer to them as filters. These are defined in the template and are not changeable by the user.
...
All we have to do is add a filter to the container before we return it. Each export type can potentially use different kinds of filters. In our case, we are exporting variations. For convenience, there is the xref:stable7@docs-raw::Item.adoc#item_builders_variationfilterbuilderfactory[VariationFilterBuilderFactory] class that has a method for each available variation filter.

So let us create an instance of `VariationFilterBuilderFactory` and request a xref:stable7@docs-raw::Item.adoc#item_variationbase_variationisactive[VariationIsActive] object. In this object, we need to specify whether we want active or inactive variations.
...
Overall, the `getCustomFilter()` method is handled identically. The only real difference in the definition is that filters do not necessarily need default values to work, since they can be manipulated by the user. Apart from that the available filters are identical.

Just to have it covered, let’s also add a custom filter. A good example is the filter xref:stable7@docs-raw::Item.adoc#item_item_itemhasids[ItemHasIds]. With this, the user will be able to provide specific IDs through the UI. Since we provide a custom filter, this time we do not have to provide a default value. However, we can do so if the use case benefits from it.
...
To enable the catalogue to handle all aspects of your export logic, you should try to avoid manipulating data outside of the catalogue scope. Since you might still need to change some details here and there, we provide multiple tools to manipulate data inside the catalogue.

The first way would be a field-specific callback. You may have noticed that the three types of fields contain the method setCallable(). Via this method, you can inject an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogtemplatefieldcallablecontract[CatalogTemplateFieldCallableContract].
This interface provides only the method `call()`. This method will be called each time this field is filled with a non-null value. In the method, you receive the raw item, so all the raw data that was loaded for a specific item in this export. In the variation export, this would be all required data for a specific variation.
...
Therefore, you can conditionally manipulate the data of the field directly in the mapping process. A common scenario for this could be the generation of an sku. Typically, what the customer maps in the catalogue is a source from which the sku should be generated, and an actual sku as fallback.
...
So let’s add this example to our sku field. First, we need our own implementation of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogtemplatefieldcallablecontract[CatalogTemplateFieldCallableContract]. For this example, we won’t actually generate an sku but rather set up the basics which would allow us to do this in a productive use case. The class could look like this:
...
Obviously, we still need to register this callback on our sku field. To do this, we just change the definition of your sku field in the template provider to look like this:
...
However, this is not meant to convert the result into a CSV, XML or another file format. If you need to do so, have a look at xref:result-converters.adoc[ResultConverters], as these will not be covered in this guide.

To understand how this works, let’s add a key to the result at the end. This will just be a string that says “We manipulated our result”. Again, we have to create our own implementation - this time of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogmutatorcontract[CatalogMutatorContract]. Our implementation should look like this:
...
Until now, we used the catalogue UI to look into the data that is exported. However, in our productive environment, we might want to trigger the export from the code. Although there are multiple ways of doing so, in this tutorial we will only have a look at one of them. The others will be covered in another tutorial.
...
Inside of the foreach, we will always have a catalogue that is matching our filters. Since we just want to have an example, we can add a return statement at the end of the foreach, so that we only export a single one for now. Additionally, we now need an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogexportrepositorycontract[CatalogExportRepositoryContract] which we can request before we enter our do while loop.
...
Let’s go through what happens here line by line. At the beginning of the do while, we load all our catalogues. We then iterate through them and request an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogexportservicecontract[CatalogExportServiceContract] through the `exportById()` method of our xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogexportrepositorycontract[CatalogExportRepositoryContract].
The comments show some examples of working with this service before triggering the actual export. What is important is that all the logic that is applied at this point and has to be done to get a valid export should be moved into the xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogdynamicconfigcontract[DynamicConfig].
This is necessary to enable exports directly out of the catalogue scope (e.g. preview or download through the UI). If you don’t provide all that logic in the xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogdynamicconfigcontract[DynamicConfig], you should set the `isPreviewable` attribute of your template to false.
An in-depth explanation of `DynamicConfig` can be found xref:dynamic-config-classes.adoc[here].


Through the `getResult()` method, we receive an instance of xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogdynamicconfigcontract[CatalogExportResult] *Link correct?*. This will actually run the mapping logic and return our results in an array. You can receive the result in chunks by iterating over the xref:stable7@docs-raw::Catalog.adoc#catalog_contracts_catalogdynamicconfigcontract[CatalogExportResult] *Link correct?*, since it implements the iterable interface.
...
=== Conclusion - change title (is level of title correct here?)
...
For further specific use cases we provide different tutorials which will help you to optimise your usage of the catalogue. Some examples are:

* xref:dynamic-config-classes.adoc[DynamicConfig]
* xref:result-converters.adoc[Result conversion]
* Async exports *ADD LINK*???
