...
As we can see here, we first define our container, then we define a group and at last a field. In general, this process will always be the same. That means we will always define groups, and we will always add fields to them. What will change depending on the use case is the type of the field. As we can see in the example, we define an instance of the xref:stable7@interface::Catalog.adoc#_catalog_models_simpletemplatefield[SimpleTemplateField] class. Depending on the specific use case, we would have to use a different one.

Currently available are:
* *xref:stable7@interface::Catalog.adoc#_catalog_models_simpletemplatefield[SimpleTemplateField]*: Defines a result field that can be filled through a source field out of the plentymarkets system or a custom value.
* *xref:stable7@interface::Catalog.adoc#_catalog_models_complextemplatefield[ComplexTemplateField]*: Defines a result field that will be filled by a value out of a predefined list depending on a condition (e.g. a specific category name depending on a linked category in plentymarkets).
* *xref:stable7@interface::Catalog.adoc#_catalog_models_combinedtemplatefield[CombinedTemplateField]*: Defines two result fields - a complex one and a simple one. The complex field will be filled depending on which source fields were used to fill the simple field (e.g. filling a barcode type depends on which source field was used to fill the barcode field).
...
A complex template field is mostly similar to the definition of a simple template field. The most important difference is that a complex template field needs an instance of xref:stable7@interface::Catalog.adoc#_catalog_contracts_catalogmappingvalueprovidercontract[CatalogMappingValueProviderContract] to define the list of mappable values. Another difference is that complex fields do not support default sources. The mapping value provider defines the possible valid values, which are the core of a complex mapping.
...
Since we’ve already covered the other two types, this one should be straightforward. It is actually a combination of the previous two types. Just as in the last step, we need an instance of xref:stable7@interface::Catalog.adoc#_catalog_contracts_catalogmappingvalueprovidercontract[CatalogMappingValueProviderContract]. This time, it will provide different types of barcodes. To keep it simple, let’s only use *GTIN* and *ISBN* for now. The provider class could look like this:
...
Additionally, we need an instance of xref:stable7@interface::Catalog.adoc#_catalog_containers_catalogtemplatefieldcontainer[CatalogTemplateFieldContainer] which will contain all simple fields of our combined field. The logic is the following:
...
All we have to do is add a filter to the container before we return it. Each export type can potentially use different kinds of filters. In our case, we are exporting variations. For convenience, there is the xref:stable7@interface::Item.adoc#_item_builders_variationfilterbuilderfactory[VariationFilterBuilderFactory] class that has a method for each available variation filter.

So let us create an instance of `VariationFilterBuilderFactory` and request a xref:stable7@interface::Item.adoc#_item_variationbase_variationisactive[VariationIsActive] object. In this object, we need to specify whether we want active or inactive variations.
