...
This how-to guide will introduce you to *DynamicConfig* classes in the catalogue. In the following explanations, we assume that you're already familiar with the knowledge from the xref:basic-usage.adoc[basic usage tutorial]. So work through this first if you haven’t already done so.
...
Another topic that might be interesting for you at this point are xref:result-converters.adoc[ResultConverters], which you can use to manipulate your result’s file type and change it for example to CSV.
