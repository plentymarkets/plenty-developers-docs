...
This tutorial will introduce you to DynamicConfig classes in the catalogue. In the following explanations, we assume that you're already familiar with the knowledge from the basic usage tutorial -INSERT LINK-. So work through this first if you haven’t already done so.
...
This would cause different export results depending on the trigger which will lead to more problems down the line. Thus, the DynamicConfig -INSERT LINK- provides a spot to inject your logic directly before the export process is triggered.
...
Another topic that might be interesting for you at this point are ResultConverters -INSERT LINK-, which you can use to manipulate your result’s file type, e.g. to CSV.
