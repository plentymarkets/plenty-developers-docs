...
To implement *DynamicConfig* in your template, just overwrite the `getDynamicConfig()` method in your template provider and create an instance of your own class that implements the xref:stable7@interface::Catalog.adoc#_catalog_contracts_catalogdynamicconfigcontract[CatalogDynamicConfigContract].

In this class you need to create two methods, `applyDynamicConfig()` and `applyDynamicPreviewConfig()`. These methods receive identical parameters: the xref:stable7@interface::Catalog.adoc#_catalog_models_catalog[catalog], the xref:stable7@interface::Catalog.adoc#_catalog_contracts_templatecontract[template] and an instance of xref:stable7@interface::Catalog.adoc#_catalog_contracts_catalogexportservicecontract[CatalogExportService].
