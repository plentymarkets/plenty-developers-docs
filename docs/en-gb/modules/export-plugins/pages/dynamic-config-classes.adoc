...
This how-to guide will introduce you to *DynamicConfig* classes in the catalogue. In the following explanations, we assume that you're already familiar with the knowledge from the xref:export-bbasic-usage.adoc[basic usage tutorial]. So work through this first if you haven’t already done so.
...
To implement *DynamicConfig* in your template, just overwrite the `getDynamicConfig()` method in your template provider and create an instance of your own class that implements the xref:stable7@docs-raw::Catalog.adoc#_catalog_contracts_catalogdynamicconfigcontract[CatalogDynamicConfigContract].

In this class you need to create two methods, `applyDynamicConfig()` and `applyDynamicPreviewConfig()`. These methods receive identical parameters: the xref:stable7@docs-raw::Catalog.adoc#_catalog_models_catalog[catalog], the xref:stable7@docs-raw::Catalog.adoc#_catalog_contracts_templatecontract[template] and an instance of xref:stable7@docs-raw::Catalog.adoc#_catalog_contracts_catalogexportservicecontract[CatalogExportService].
