...
Our plugin consists of link:/dev-doc/basics#plugin-information[plugin.json] and the link:/dev-doc/basics#plugin-options[config.json]. When interacting with an API, which will surely be necessary in most cases, you will also need the classes required for this communication.
...
After creating the plugin and configuring the settings in the configuration, we have to link:/dev-doc/basics#add-repository[add] our new plugin to the plentymarkets inbox.
