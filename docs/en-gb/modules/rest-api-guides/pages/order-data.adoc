...
The `metaData` of the variation order item must contain a random but unique `externalHash`, so the order item can be references correctly. The `metaData` of the orderProperty order item must have an `internalReferences` entry with `internalReferences` as key and the corresponding `externalHash` value.
