...
The `locationId` gives you the ID of the accounting location that is used for the order. Use link:/rest-doc/accounting/details#list-accounting-locations-of-a-client[this route] the list the accounting locations of a client.
...
The `properties` relation holds various order information. It has a simple key/value structure. The `typeId` is the key. This key is an integer and the `value` a string. Use link:/rest-doc/order_order_property/details#list-order-property-types[this route] to get all types. The shipping profile e.g. is stored in a order property (`typeId` = 2).

=== Dates

The `dates` relation works similarly to the `properties` relation, but stores dates instead of strings. Use link:/rest-doc/order_order_date/details#list-order-date-types[this route] to get all types.
...
The `referrerId` describes where an order item comes from. Use link:/rest-doc/order_order_referrer/details#list-referrers[this route] the get a list of the referrers available in the system.
...
The tax information is stored in the values `countryVatId`, `vatField` and `vatRate`. The `countryVatId` is the ID of a vat configuration in the system. Use link:/rest-doc/accounting_vat/details[these routes] to get vat configurations. Each vat configuration has 4 vat fields. The `vatField` specifies one of the 4 vat fields of that configuration. Thus values from 0 to 3 are possible. `vatRate` is the concrete vat rate in percent.
