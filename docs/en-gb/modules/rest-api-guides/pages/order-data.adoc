...
The order item `references` relation is similar to `orderReferences` relation. Order items can reference other order items. 
...
The new order item orderProperties are realized as order items with `typeId` 15. The can have own prices and tax rates, depending of the property definition. Also they should have one of the new order property types:
...
As an order item orderProperty belongs to a variation order item (`typeId == 1`), a reference between the orderProperty and the variation has to be created. For this, `metaData` have to be added in the variation and the orderProperty order item.

The `metaData` of the variation order item must contain a random but unique `externalHash`, so the order item cann be references correctly. The `metaData` of the orderProperty order item must have an `internalReferences` entry with `internalReferences` as key and the corresponding `externalHash` value.

The `orderItemName` should contain:

* the property name, if the property does not belong to a group with `select` oder `multiselect` as `groupType`.
* the property group name, if the property belongs to a group with `select` oder `multiselect` as `groupType`. In the case of `select` the order item property with `typeId == 82` contains the selected value(s) as a JSON string.
