...
We want to enhance your workflow with our API. For that reason, we built a whole new internal annotation progress. At the moment, we deliver a file you can use with your preferred API software. You can checkout our REST-Documentation, based upon the Swagger-UI, link:/rest-doc[here].


You either can download it or check out our GitHub repository.

https://github.com/plentymarkets/api-doc/[GitHub]
link:/downloads/plentymarkets-openapi?type=plentymarkets[Download]
...
The API uses the "Bearer" authentication scheme to transmit the access token. To get the token for authentication, use the link:/rest-doc/authentication#login[Login] call. By executing this POST method with a valid body, it will return a JSON object with the necessary data.
...
Bulk REST routes help you to work efficiently within the throttling limits when handling large amounts of data. Check out the https://developers.plentymarkets.com/tutorials/bulk-routes[tutorial on bulk routes] to find out which bulk routes are currently available.
