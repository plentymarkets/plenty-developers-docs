...
In the example above we specify the template path of the new component like this:
`{{ component( "Theme::content.Components.ThemeBasketList" ) }}`

When including the BasketList template we use the internal id of the template as attribute value:
`<basket-list template="#theme-basket-list"></basket-list>`

We will create the BasketList template in the next step.
