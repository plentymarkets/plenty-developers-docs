...
* link:#widget-folder-structure[Widget plugin folder structure]
* link:#contentwidgets[Description of the contentWidgets.json]
* link:#widget-input-types[Widget input types]
* link:#widget-isvisible-property[The isVisible property]
* link:#widget-islist-property[The isList property]
* link:#widget-php-class[The widget PHP class]
* link:#widget-twig-chapter[The widget TWIG]
...
* *type*: Specifies the input type of the widget setting. Please find a detailed explanation of the various input types link:#widget-input-types[further down the page].
* *required*: A Boolean that determines whether this widget setting is mandatory for the user.
* *defaultValue*: Determines the default value for a setting. The type of value is contingent on the input type. Please find a detailed description of applicable default values for each input type further link:#widget-default-values[below].
...
* *isVisible*: Determines whether the setting is visible. If nothing else is specified, the default value is "true". You can define a JavaScript expression, like an if-condition, that dynamically changes the value of *isVisible*. An example of how this setting is implemented is described further link:#widget-isvisible-property[below].
* *isList*: Determines whether the setting can be duplicated. This setting can be used, for instance, to add further slides to the image carousel or add additional entries to the list widget. Please find a detailed explanation of how to implement this setting further link:#widget-islist-property[below].
